...
class ApiTestCase(TestCase):
    ...
    def test_canonical_resource_for(self):
        ...
        self.assertEqual(
            isinstance(api.canonical_resource_for("notes"), NoteResource), True
        )
    ...
    def test_urls(self):
        ...
        self.assertEqual(
            sorted([pattern.name for pattern in patterns if hasattr(pattern, "name")]),
            ["api_v1_top_level"],
        )
        self.assertEqual(
            [
                [
                    pattern.name
                    for pattern in include.url_patterns
    ...
    def test_top_level(self):
        ...
        self.assertEqual(
            ...
            '{"notes": {"list_endpoint": "/api/v1/notes/", "schema": "/api/v1/notes/schema/"}, "users": {"list_endpoint": "/api/v1/users/", "schema": "/api/v1/users/schema/"}}',
        )
    ...
    def test_top_level_include_schema_content(self):
        ...
        notes_schema = adjust_schema(
            json.loads(note_resource.get_schema(dummy_request).content.decode("utf-8"))
        )
        user_schema = adjust_schema(
            json.loads(user_resource.get_schema(dummy_request).content.decode("utf-8"))
        )
    ...
    def test_custom_api_serializer(self):
        ...
        self.assertEqual(
            resp["content-type"],
            "application/json",
            msg="Expected application/json response but received %s"
            % resp["content-type"],
        )
