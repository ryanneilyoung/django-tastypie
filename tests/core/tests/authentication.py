...
class BasicAuthenticationTestCase(TestCase):
    ...
    def test_is_authenticated(self):
        ...
        john_doe.save()
        request.META["HTTP_AUTHORIZATION"] = "Basic %s" % base64.b64encode(
            "johndoe:pass:word".encode("utf-8")
        ).decode("utf-8")
        self.assertEqual(auth.is_authenticated(request), True)
...
class DigestAuthenticationTestCase(TestCase):
    ...
    def test_is_authenticated(self):
        ...
        self.assertEqual(
            auth_request["WWW-Authenticate"].find(' realm="django-tastypie"') > 0, True
        )
...
@skipIf(
    settings.DJANGO_VERSION >= settings.DJANGO_11,
    "oauth-plus not compatible with django 1.11",
)
class OAuthAuthenticationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.token_inactive, _ = Token.objects.get_or_create(
            key="bar",
            token_type=Token.ACCESS,
            defaults={
                "consumer": self.consumer,
...
class MultiAuthenticationTestCase(TestCase):
    ...
    def test_apikey_and_authentication_enforce_user(self):
        ...
        self.assertEqual(
            isinstance(api_key_auth.is_authenticated(request2), HttpUnauthorized), True
        )
