...
class PerUserAuthorizationTestCase(ResourceTestCaseMixin, TestCase):
    def setUp(self):
        ...
        self.site_1 = Site.objects.create(
            domain="superawesomenewssite.com", name="Super Awesome News Site"
        )
        self.site_2 = Site.objects.create(
            domain="snarkynewssite.com", name="Snarky News Site"
        )

        self.user_1 = User.objects.create_user(
            "mr_author", "mister_author@example.com", "pass"
        )
        self.user_2 = User.objects.create_user(
            "mrs_author", "missus_author@example.com", "pass"
        )
        self.user_3 = User.objects.create_user(
            "ms_editor", "miss_editor@example.com", "pass"
        )
        ...
        self.author_uri_1 = "/api/v1/authorprofile/{0}/".format(
            self.author_profile_1.pk
        )
        self.author_uri_2 = "/api/v1/authorprofile/{0}/".format(
            self.author_profile_2.pk
        )
        self.author_uri_3 = "/api/v1/authorprofile/{0}/".format(
            self.author_profile_3.pk
        )

    def test_get_list(self):
        # Should be all articles.
        resp = self.api_client.get(
            "/api/v1/article/", format="json", authentication=self.author_auth_1
        )
        ...
        resp = self.api_client.get(
            "/api/v1/article/", format="json", authentication=self.author_auth_2
        )
        self.assertValidJSONResponse(resp)
        self.assertEqual(len(self.deserialize(resp)["objects"]), 3)
        first_article = self.deserialize(resp)["objects"][0]
    ...
    def test_get_detail(self):
        # Should be all articles.
        resp = self.api_client.get(
            self.article_uri_1, format="json", authentication=self.author_auth_1
        )
        self.assertValidJSONResponse(resp)
        first_article = self.deserialize(resp)
        self.assertKeys(
            first_article,
            ["added_on", "authors", "content", "id", "resource_uri", "slug", "title"],
        )
        self.assertEqual(first_article["id"], self.article_1.pk)

        # Should ALSO be all articles, even though it's not our article.
        resp = self.api_client.get(
            self.article_uri_1, format="json", authentication=self.author_auth_2
        )
        self.assertValidJSONResponse(resp)
        first_article = self.deserialize(resp)
        self.assertKeys(
        ...
        resp = self.api_client.get(
            self.article_uri_2, format="json", authentication=self.author_auth_1
        )
        self.assertValidJSONResponse(resp)
    ...
    def test_put_list(self):
        ...
        the_data["objects"][2]["title"] = "Updated: {0}".format(
            the_data["objects"][2]["title"]
        )
        ...
        self.assertHttpAccepted(
            self.api_client.put(
                ...
                format="json",
                data=the_data,
                authentication=self.author_auth_3,
            )
        )
        # Verify no change in count.
        self.assertEqual(Article.objects.count(), 3)
        self.assertEqual(
            Article.objects.get(pk=self.article_1.pk).title, "This is edited."
        )
        self.assertEqual(
            Article.objects.get(pk=self.article_1.pk).content,
            "Some big tech company announced new stuffs! Go get your consumerism on!",
        )
        self.assertEqual(
            Article.objects.get(pk=self.article_3.pk).title,
            "Updated: Ugh, Who Cares About New Stuff?",
        )
        self.assertEqual(
            Article.objects.get(pk=self.article_3.pk).content,
            "Obviously stuff by other by other company is way better.",
        )
    ...
    def test_put_detail(self):
        ...
        self.assertEqual(Article.objects.count(), 3)
        self.assertEqual(
            Article.objects.get(pk=self.article_1.pk).title, "Revised Story"
        )
        self.assertEqual(
            Article.objects.get(pk=self.article_1.pk).content,
            "We didn't like the previous version.",
        )
        ...
        self.assertEqual(
            Article.objects.get(pk=self.article_2.pk).title,
            "Editorial: Why stuff is great",
        )
        self.assertEqual(
            Article.objects.get(pk=self.article_2.pk).content,
            "Because you can buy buy buy & fill the gaping voids in your life.",
        )
    ...
    def test_delete_list(self):
        ...
        self.assertHttpUnauthorized(
            self.api_client.delete(
                "/api/v1/article/", format="json", authentication=self.author_auth_3
            )
        )
        self.assertEqual(Article.objects.count(), 3)
    ...
    def test_delete_detail(self):
        ...
        self.assertHttpUnauthorized(
            self.api_client.delete(
                self.article_uri_1, format="json", authentication=self.author_auth_3
            )
        )
        self.assertEqual(Article.objects.count(), 3)
