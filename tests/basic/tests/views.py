...
class ViewsTestCase(TestCaseWithFixture):
    def test_gets(self):
        ...
        self.assertEqual(
            deserialized["notes"],
            {"list_endpoint": "/api/v1/notes/", "schema": "/api/v1/notes/schema/"},
        )
    ...
    def test_posts(self):
        ...
        resp = self.client.post(
            "/api/v1/notes/", data=post_data, content_type="application/json"
        )
    ...
    def test_puts(self):
        ...
        resp = self.client.put(
            "/api/v1/notes/1/", data=post_data, content_type="application/json"
        )
    ...
    def test_slugbased(self):
        ...
        self.assertEqual(
            [obj["title"] for obj in deserialized["objects"]],
            [u"First Post", u"Another First Post"],
        )
        ...
        resp = self.client.get(
            "/api/v2/slugbased/set/another-first-post;first-post/",
            data={"format": "json"},
        )
        self.assertEqual(resp.status_code, 200)
        deserialized = json.loads(resp.content.decode("utf-8"))
        ...
        self.assertEqual(
            [obj["title"] for obj in deserialized["objects"]],
            [u"Another First Post", u"First Post"],
        )
    ...
    def test_session_auth(self):
        ...
        resp = csrf_client.get(
            "/api/v2/sessionusers/",
            data={"format": "json"},
            HTTP_X_CSRFTOKEN="o9nXqnrypI9ydKoiWGCjDDcxXI7qRymH",
        )
        self.assertEqual(resp.status_code, 200)
        deserialized = json.loads(resp.content.decode("utf-8"))
        self.assertEqual(len(deserialized), 2)
...
class MoreViewsTestCase(TestCaseWithFixture):
    ...
    def test_vary_accept(self):
        ...
        response = self.client.get(
            "/api/v1/cached_users/", HTTP_ACCEPT="application/json"
        )
    ...
    def test_cache_control(self):
        ...
        cache_control = set(
            [
                x.strip().lower()
                for x in response["Cache-Control"].split(",")
                if x.strip()
            ]
        )
    ...
    def test_cache_disabled_control(self):
        response = self.client.get(
            "/api/v1/cache_disabled_users/", HTTP_ACCEPT="application/json"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_public_cache_control(self):
        response = self.client.get(
            "/api/v1/public_cached_users/", HTTP_ACCEPT="application/json"
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(
            cache_control, set(["s-maxage=3600", "max-age=3600", "public"])
        )
        self.assertTrue('"johndoe"' in response.content.decode("utf-8"))

    def test_private_cache_control(self):
        response = self.client.get(
            "/api/v1/private_cached_users/", HTTP_ACCEPT="application/json"
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(
            cache_control, set(["s-maxage=3600", "max-age=3600", "private"])
        )
        self.assertTrue('"johndoe"' in response.content.decode("utf-8"))
