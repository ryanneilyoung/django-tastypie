...
class FilteringErrorsTestCase(TestCaseWithFixture):
    def test_valid_date(self):
        resp = self.client.get(
            "/api/v1/notes/",
            data={"format": "json", "created__gte": "2010-03-31 00:00:00Z"},
        )
        self.assertEqual(resp.status_code, 200)
        deserialized = json.loads(resp.content.decode("utf-8"))
        self.assertEqual(
            len(deserialized["objects"]),
            Note.objects.filter(created__gte="2010-03-31 00:00:00Z").count(),
        )

    def test_invalid_date(self):
        resp = self.client.get(
            "/api/v1/notes/", data={"format": "json", "created__gte": "foo-baz-bar"}
        )
...
class PostRelatedUrlValidationTestCase(TestCaseWithFixture):
    def test_valid_url(self):
        ...
        resp_with_pk = self.client.post(
            "/api/v1/notes/", data=data_with_pk, content_type="application/json"
        )
        self.assertEqual(resp_with_pk.status_code, 201)
        note_posted_with_pk = json.loads(
            self.client.get(resp_with_pk["location"]).content.decode("utf-8")
        )
        resp_with_url = self.client.post(
            "/api/v1/notes/", data=data_with_url, content_type="application/json"
        )
        self.assertEqual(resp_with_url.status_code, 201)
        note_posted_with_url = json.loads(
            self.client.get(resp_with_url["location"]).content.decode("utf-8")
        )
    ...
    def test_invalid_url(self):
        ...
        with self.assertRaises(NotFound):
            self.client.post(
                "/api/v1/notes/", data=data, content_type="application/json"
            )
...
class PostNestResouceValidationTestCase(TestCaseWithFixture):
    ...
    def test_invalid_data(self):
        ...
        self.assertEqual(
            ...
            {
                "notes": {"title": ["This field is required."]},
                "annotated": {"annotations": ["This field is required."]},
            },
        )
...
class PutDetailNestResouceValidationTestCase(TestCaseWithFixture):
    def test_valid_data(self):
        ...
        resp = self.client.put(
            "/api/v1/notes/1/", data=data, content_type="application/json"
        )
        self.assertEqual(resp.status_code, 204)
        note = json.loads(
            self.client.get(
                "/api/v1/notes/1/", content_type="application/json"
...
class PutListNestResouceValidationTestCase(TestCaseWithFixture):
    def test_valid_data(self):
        ...
        note = json.loads(
            self.client.get(
                "/api/v1/notes/2/", content_type="application/json"
            ).content.decode("utf-8")
        )
        self.assertTrue(note["annotated"])
    ...
    def test_invalid_data(self):
        ...
        self.assertEqual(
            json.loads(resp.content.decode("utf-8")),
            {
                "notes": {"content": ["This field is required."]},
                "annotated": {"annotations": ["This field is required."]},
            },
        )
