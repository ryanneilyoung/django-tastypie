...
from related_resource.models import (
    Category,
    Label,
    Tag,
    Taggable,
...
class CategoryResourceTest(TestCaseWithFixture):
    def setUp(self):
        ...
        self.child_cat_2 = Category.objects.create(
            parent=self.parent_cat_2, name="Daughter"
        )
    ...
    def test_put_null(self):
        ...
        self.assertEqual(
            Category.objects.get(pk=self.child_cat_1.pk).parent.pk, self.parent_cat_1.pk
        )
...
class ExplicitM2MResourceRegressionTest(TestCaseWithFixture):
    def setUp(self):
        ...
        self.taggabletag_1 = TaggableTag.objects.create(
            tag=self.tag_1, taggable=self.taggable_1
        )
...
class RelationshipOppositeFromModelTestCase(TestCaseWithFixture):
    ...
    def test_create_similar(self):
        ...
        data = {"name": "OtherJob", "payment": {"scheduled": self.some_time_str}}
...
class NestedRelatedResourceTest(TestCaseWithFixture):
    ...
    def test_many_to_one(self):
        ...
        data = {
            "name": "Joan Rivers",
            "dogs": [{"name": "Snoopy", "house": {"color": "Red"}}],
        }
    ...
    def test_many_to_many(self):
        ...
        data = {
            "name": "Joan Rivers",
            "dogs": [{"name": "Snoopy", "bones": [{"color": "white"}]}],
        }
...
class CorrectUriRelationsTestCase(TestCaseWithFixture):
    ...
    def test_incorrect_uri(self):
        ...
        User.objects.create(
            id=note_1.pk, username="valid", email="valid@exmaple.com", password="junk"
        )

        data = {
            # This URI is flat-out wrong (wrong resource).
            # This should cause the request to fail.
            "author": "/v1/notes/{0}/".format(note_1.pk),
            "title": "Nopenopenope",
        ...
        self.assertEqual(
            str(cm.exception),
            "An incorrect URL was provided '/v1/notes/2/' for the 'UserResource' resource.",
        )
        self.assertEqual(Note.objects.count(), 2)
...
class ModelWithReverseItemsRelationshipTest(TestCase):
    def test_reverse_items_relationship(self):
        ...
        request.path = reverse(
            "api_dispatch_list",
            kwargs={
                "resource_name": order_resource._meta.resource_name,
...
class OneToOneTestCase(TestCase):
    def test_reverse_one_to_one_post(self):
        ...
        request.body = json.dumps(
            {"name": "tag_name", "tagged": [], "extradata": "/v1/extradata/%s/" % ed.pk}
        )
    ...
    def test_toonefield_spanning_a_relationship(self):
        ...
        class CustomPersonResource(PersonResource):
            company_address = fields.ToOneField(
                AddressResource, "company__address", null=True, full=True
            )
